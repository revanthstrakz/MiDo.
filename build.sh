...
export FINAL_ZIP="${ZIPDIR}/Feather-mido-treble-clang-$(date +"%Y%m%d")-$(date +"%H%M%S").zip"
...
export FINAL_ZIP="${ZIPDIR}/Feather-mido-treble-$(date +"%Y%m%d")-$(date +"%H%M%S").zip"
...
if [ "$CLANG" == "yes" ]
then
sendTG "Starting $(date +%Y%m%d) Feather Treble Clang [build]($BUILD_URL)."
make CC=clang -j16 O=out/
else
sendTG "Starting $(date +%Y%m%d) Feather Treble [build]($BUILD_URL)."
make -j8 O=out/
fi
END=$(date +"%s")
DIFF=$((END - START))
